class ChristmasLights{
    constructor(){ //creates a serie of properties of the balls
        this.ballId;
        this.colorname; 
        this.speed;
        this.intensity;
...
updateSpeedClass(){
    ...
    balls.forEach( (ball) => {
        if (this.checkCSS(ball.id)){
            let customClass = findKeyClass(`class-${ball.id}`);
...
createRules(){
    ...
    if (checkCSS(this.ballId)) { //if the chosen ball already has a class, remove the previous class and the animation
